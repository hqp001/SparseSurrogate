...
def prune_and_fine_tune_lottery_ticket(model, X_train, y_train, initial_state_dict, n_rounds, total_sparsity, n_epochs=10, lr=0.01):
    ...
    for round in range(n_rounds):
        # Restore model to its initial state before pruning
...
def build_and_optimise_adversarial_mnist_torch(
    ...
    # Apply Lottery Ticket Hypothesis Pruning and Fine-tuning
    initial_state_dict = reg.state_dict()  # Save initial weights
    X_train, y_train = next(iter(train_dataloader))  # Get a batch of training data
    total_sparsity = 0.5  # Example sparsity level
    n_rounds = 10  # Example number of pruning rounds
    reg = prune_and_fine_tune_lottery_ticket(reg, X_train, y_train, initial_state_dict, n_rounds, total_sparsity, n_epochs=10, lr=0.01)
