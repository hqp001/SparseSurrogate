...
def formulate_double(
    ...
    output_dense_vars = np.zeros((10,), dtype=object)
    for i in range(10):
        output_dense_vars[i] = scip.addVar(name=f"y_dense_{i}", vtype="C", lb=None, ub=None)

    output_sparse_vars = np.zeros((10, ), dtype=object)
    for i in range(10):
        output_sparse_vars[i] = scip.addVar(name=f"y_sparse_{i}", vtype="C", lb=None, ub=None)
    ...
    scip.setObjective(-output_dense_vars[wrong_label] + output_dense_vars[right_label] + 1)
    ...
    pred_cons = add_predictor_constr(
        ...
        input_vars,
        output_sparse_vars,
        unique_naming_prefix="sparse_adversarial_",
        formulation=formulation,
    )

    pred_dense_cons = add_predictor_constr(
        scip,
        dense_model,
        input_vars,
        output_dense_vars,
        unique_naming_prefix="dense_adversarial_",
        formulation=formulation
    )
