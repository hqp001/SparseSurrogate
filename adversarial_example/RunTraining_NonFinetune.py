...
def run_training():
    ...
    for args in generate_arguments():
        ...
        for sparsity in [0.3, 0.5, 0.8, 0.9, 0.95, 0.99]:
            for structured in [True]:
                sparse_model = copy.deepcopy(model)
                sparse_model, test_score = prune(sparse_model, args['n_pixel_1d'], sparsity, n_rounds=ROUNDS, max_epochs=PRUNE_EPOCHS, structured=structured)
                output_args = {
                ...
                manager.insert_model(input_id=input_id, model_name=f'non_finetune_structure_{sparsity}', output_args=output_args, model=sparse_model)
